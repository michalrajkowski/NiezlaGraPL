# Script made by modifying the text_box script and combining it with pilmoji
# Original script by digitaltembo
# Github link: https://gist.github.com/digitaltembo/eb7c8a7fdef987e6689ee8de050720c4

""" 
The text can also include discord emojies for example: <:hp:836346927963570207> as HP emoji. 
...
def emoji_text_box(text, image, font, box, horizontal_allignment = ALLIGNMENT_LEFT, vertical_allignment = ALLIGNMENT_TOP, **kwargs):
    # Extract discord emoji codes from text and store them in temporary list
    # Insert any emote in text as its replacement

    pattern = r"<:[a-zA-Z0-9_]+:(\d+)>"
    emote_symbol = "🔴" # This can be any emote, i chose red circle for some reason
    matches = [(match.group(), match.start(), match.end()) for match in re.finditer(pattern, text)]
    replaced_text = text
    for match, start, end in reversed(matches):
        replaced_text = replaced_text[:start] + emote_symbol + replaced_text[end:]
    ...
    # Put back discord emoji codes to text
    match_texts = [match[0] for match in matches]
    matched_index = 0
    new_lines = []
    for i, line in enumerate(true_lines):
        start = 0  
        while emote_symbol in line[start:]:
            pos = line.find(emote_symbol, start)
            
            line = line[:pos] + match_texts[matched_index] + line[pos + len(emote_symbol):]
            start = pos + len(match_texts[matched_index])
            matched_index+=1
    ...
    with Pilmoji(image) as pilmoji:
        for line in true_lines:
            ...
            pilmoji.text((int(x + x_offset), int(y + y_offset)),
                line,
                font=font,
                **kwargs)
            """image_draw.text(
